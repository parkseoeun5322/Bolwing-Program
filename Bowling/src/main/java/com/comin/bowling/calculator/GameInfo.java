...
public class GameInfo {
	...
	public GameVO gameInfoCalc(GameVO gvo, int rollNum, Integer ball_10_1) {
		...
		if(frame < 10) {
			if(turn == 1 && remainPin != 0) {	//첫번째 투구 시 스트라이크 X
				turn = 2;
			} else if(turn == 1 && remainPin == 0) {	//첫번재 투구 시 스트라이크 일때
				turn = 1;
				pseq++;
				remainPin = 10;
			} else if(turn == 2) {	//두번째 투구까지 쳤을 때
				turn = 1;
				pseq++;
				remainPin = 10;
			}
	...
	public GameVO gameInfoDeleteCalc(GameVO gvo, List<PlayerVO> list) {
		...
		// 플레이어 순서가 1일 경우 마지막 플레이어로 변경, 프레임은 감소시키고 플레이어 순서가 1이 아닐 경우에는 순서를 차례로 감소
		if(turn == 1) {
			if(pseq != 1) {
				pseq--;
				if(frame == 10) {
		...
		if(frame < 10) {
			...
			if(turn == 1) {
				if(ball_10_3 != null) {		//10프레임의 3투구(보너스)를 지울때
					...
					else remainPin = 10 - ball_10_2;	//10프레임의 1투구는 스트라이그지만 2투구가 스트라이크 X
